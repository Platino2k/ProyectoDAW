...
function LOADTOWN($db, $world, $lang, $buildingCOST, $armyCOST)
{
    ...
    for ($i = 0; $i < 4; $i++) {
        ...
        if (!empty($buildingCOST[$nextBuilding[$i]])) {
            ...
            if (
                $result3["FOOD"] >= $buildingCOST[$nextBuilding[$i]][0] &&
...
function SHOWMAP($db, $world, $lang)
{
    ...
    if (empty($result4['SWORDMAN'])) {
        $result4['SWORDMAN'] = 0;
    }
    if (empty($result4['PIKEMAN'])) {
        $result4['PIKEMAN'] = 0;
    }
    if (empty($result4['ARCHER'])) {
        $result4['ARCHER'] = 0;
    }
    if (empty($result4['L_CAVALRY'])) {
        $result4['L_CAVALRY'] = 0;
    }
    if (empty($result4['H_CAVALRY'])) {
        $result4['H_CAVALRY'] = 0;
    }
